...
bool initialize_sdl()
 {
	...
	{
		//Use OpenGL 2.1, Set double buffering, Set Depth buffer size
		...
		//Create window
		mainWindow = SDL_CreateWindow("Final Project", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
 ...
 void renderNormal(PVector origin, PVector norm, float size)
 {
...
void display(timePassed){
	...
	glEnable(GL_DEPTH_TEST);
	// Makes it so lighting isn't dependent on different magnitude normals
    glEnable(GL_NORMALIZE);
...
void key()
  {
  	isPressed = SDL_GetKeyboardState(NULL);
	//Change modes
	//Z-buffer
	if(isPressed[SDL_SCANCODE_1])
	{
	 mode = 1;
	 Project();
	}
	if(isPressed[SDL_SCANCODE_2])
	{
	 mode = 2;
	 Project();
	}
	if(isPressed[SDL_SCANCODE_3])
	{
	 mode = 3;
	 Project();
	}
	...
	while(quit == false)
	{
		...
		while(SDL_PollEvent(&e) != 0)
		{
			...
			if((e.type == SDL_KEYDOWN) && (e.key.keysym.sym == SDLK_ESCAPE))
				{quit = true;}
			// If the window gets resized event
		...
		//Get time
		float timePassed = physicsTimer.get_ticks()/1000.0f;


		//Computation
		time_step(timePassed);
		...
		if(fps.get_ticks() < 1000 / FRAMES_PER_SEC)
			{SDL_Delay((1000 / FRAMES_PER_SEC) - fps.get_ticks());}
